...
export const ProofOfWork: React.FC = () => {
  ...
  const fetchImpressions = async (usernameToFetch: string) => {
    ...
    try {
      ...
      if (!res.ok || !json.ok) {
        setError(json.error || 'Failed to fetch Arc metrics. Please try again.');
        return;
      }
  ...
  const handleDownloadPNG = async () => {
    ...
    try {
      ...
      if (blob) {
        ...
        a.href = url;
        a.download = `${data.profile.screen_name}-arc-proof.png`;
  ...
  const shareText = data
    ? encodeURIComponent(
        `I generated ${data.total_impressions.toLocaleString()} impressions contributing to @arc Network ($ARC)! ⚡️\n\nVerify your Arc Proof-of-Work: #BuildOnArc #ArcNetwork`
      )
    : '';

  // Render SVG Cumulative Sparkline
    ...
    return (
      <div className="sparkline-wrapper">
        <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
          <defs>
            <linearGradient id="arc-spark-grad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#2563EB" stopOpacity="0.28" />
              <stop offset="100%" stopColor="#2563EB" stopOpacity="0.0" />
            </linearGradient>
          </defs>
          <path d={areaD} fill="url(#arc-spark-grad)" />
          <path
            d={pathD}
            fill="none"
            stroke="#2563EB"
    ...
    <div className="pow-box">
      {!data && !loading && (
        <>
          <div style={{ textAlign: 'center', marginBottom: '24px' }}>
            ...
            <p style={{ color: 'var(--arc-sky-sync)', fontSize: '14px', marginTop: '6px' }}>
              Measure and showcase your social contributions to Arc Network.
            </p>
          </div>
      ...
      {loading && (
        ...
        >
          ...
          <p style={{ fontSize: '14px', fontWeight: '500', color: 'var(--arc-sky-sync)', marginTop: '12px', fontFamily: 'var(--font-mono)' }}>
            Auditing Arc Network onchain & social proof...
          </p>
        </div>
      )}
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <span style={{ fontSize: '12px', color: 'var(--arc-cyan)', fontWeight: '700', fontFamily: 'var(--font-mono)' }}>
              ...
              <p style={{ fontSize: '13px', color: 'var(--arc-static-ether)', lineHeight: '1.45', whiteSpace: 'pre-line' }}>
                {bestPost.text}
              </p>

              <div style={{ display: 'flex', gap: '14px', marginTop: '10px', fontSize: '11px', color: 'var(--arc-text-muted)' }}>
